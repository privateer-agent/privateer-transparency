...
chatGraphSchema.methods.getFullStructure = async function() {
  ...
  const nodes = await ChatNode.find({ graphId: this._id, isActive: true })
    .select('messageCount nodeType visualMetadata connectedEdgeIds lastActivity prompt aiResponse encryptedPrompt encryptedAiResponse encryptedSources encryptedResearchTrail encryptedNoteBody imageUrl imageAttachments videoAttachments fileAttachments')
    .exec();

...
chatGraphSchema.methods.getFullStructure = async function() {
  ...
  const nodes = await ChatNode.find({ graphId: this._id, isActive: true })
    .select('messageCount nodeType visualMetadata connectedEdgeIds lastActivity prompt aiResponse encryptedPrompt encryptedAiResponse encryptedSources encryptedResearchTrail encryptedMedia encryptedNoteBody encryptedMediaDescription imageUrl imageAttachments videoAttachments fileAttachments modelId')
    .exec();
